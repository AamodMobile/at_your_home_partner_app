...
class BookingListController extends GetxController implements GetxService {
  ...
  var acceptedList = <BookingData>[];
  var startedList = <BookingData>[];
    ...
    try {
      ...
      if (apiResponse.status!) {
        ...
        for (int i = 0; i < bookingList.length; i++) {
          if (bookingList[i].status == "requested") {
            pendingList.add(bookingList[i]);
          } else if (bookingList[i].status == "accepted") {
            acceptedList.add(bookingList[i]);
          } else if (bookingList[i].status == "started") {
            startedList.add(bookingList[i]);
